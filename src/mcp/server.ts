...
// Cloud API client is optional — only used with XSpan Premium
    ...
    try {
      switch (name) {
        ...
        case 'xspan_log_nutrition':
          if (!apiClient) return errorResponse('Nutrition parsing requires XSpan Premium (cloud API). Log meals manually.');
        ...
        case 'xspan_get_health_passport':
          if (!apiClient) return errorResponse('Health Passport requires XSpan Premium (cloud API).');
          return await handleGetHealthPassport(store, apiClient, args as { week_ending?: string });

        case 'xspan_get_risk_scores':
          if (!apiClient) return errorResponse('Risk scores require XSpan Premium (cloud API).');
          return await handleGetRiskScores(apiClient);

        case 'xspan_ask_health':
          if (!apiClient || !pipeline) return errorResponse('Health Q&A requires XSpan Premium (cloud API).');
